  ...
  const loadCategory = async () => {
    try {
      ...
      if (response.successData) {
        setCategory(response.successData);
      }
    } catch (e) {
  ...
  const loadPrompts = async () => {
    try {
      ...
      const [promptsResponse, countResponse] = await Promise.all([
        promptService.getPromptsByCategory(categoryId, skip, top),
        promptService.getPromptsCountByCategory(categoryId),
      ]);
      ...
      await commonService.showSweetAlertToast({
        icon: "error",
        title: "Error",
        text: error instanceof Error ? error.message : "Failed to load prompts",
      });
    } finally {
  ...
  const handleDelete = async (id: number, title: string) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const countResponse = await promptService.getPromptsCountByCategory(categoryId);
        if (countResponse.successData) {
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                <div>
                  <h1 className="display-5 fw-bold mb-2">
                    ...
                    Prompts in &quot;{categoryName}&quot;
                  </h1>
                  <p className="text-muted mb-0">Manage prompts for this category</p>
                </div>
                <div className="d-flex gap-2">
                  <Link
                    href={`/admin/categories/${categoryId}/prompts/create`}
                        ...
                        <div className="card-body p-4 d-flex flex-column">
                          ...
                          <div className="border-top pt-3">
                            <div className="btn-group w-100" role="group">
                              <Link
                                href={`/admin/categories/${categoryId}/prompts/edit/${prompt.id}`}
            ...
            <div className="col-12">
              ...
              >
                <div className="card-body">
                  ...
                  <h5 className="mt-3 mb-2">No prompts in this category</h5>
                  <p className="text-muted mb-4">
                    {searchTerm
                      ? "Try adjusting your search"
                      : "Add your first prompt for this category"}
                  </p>
                  {!searchTerm && (
                    <Link
                      href={`/admin/categories/${categoryId}/prompts/create`}
