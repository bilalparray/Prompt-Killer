...
export default function ViewTrendingPromptPage() {
  const params = useParams();
  const id = parseInt(params.id as string);
  const [prompt, setPrompt] = useState<TrendingPromptSM | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const client = new TrendingPromptClient(storageService, storageCache, commonResponseCodeHandler);
        const service = new TrendingPromptService(client);
        const resp = await service.getTrendingPromptByIdForUser(id);
      ...
      <UserLayout>
        <div className="container py-5">
          <div className="text-center py-5">
            ...
            <Link href="/trending" className="btn btn-primary rounded-pill mt-3">Back to Trending</Link>
          </div>
        </div>
      </UserLayout>
    );
        ...
        <div className="card border-0 shadow-sm" style={{ borderRadius: "20px" }}>
          <div className="card-body p-4 p-lg-5">
            <div className="d-flex flex-wrap gap-2 mb-3">
              ...
              <span className="badge bg-warning text-dark"><i className="bi bi-fire me-1"></i>Trending</span>
              {prompt.likesCount > 0 && (
            ...
            <hr className="my-4" />
            <Link href="/trending" className="btn btn-primary rounded-pill">
              <i className="bi bi-arrow-left me-2"></i>More Trending
            </Link>
            <Link href="/home" className="btn btn-outline-secondary rounded-pill ms-2">Home</Link>
          </div>
        </div>
      </div>
    </UserLayout>
